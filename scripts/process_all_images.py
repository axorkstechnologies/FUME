...
print(f"Processing {len(matches)} matched bottle images...")
for file_name, product_id in matches.items():
    ...
    with Image.open(src_path) as img:
        ...
        webp_path = os.path.join(out_dir, f"{product_id}.webp")
        jpg_path = os.path.join(out_dir, f"{product_id}.jpg")
        
        resized.save(webp_path, "WEBP", quality=90, method=6)
        resized.save(jpg_path, "JPEG", quality=92, optimize=True)
        print(f"  [OK] {file_name} -> {product_id}.webp and .jpg ({TARGET_W}x{TARGET_H})")
...
if os.path.exists(campaign_src):
    with Image.open(campaign_src) as img:
        ...
        print("  [OK] Processed campaign-brand.webp and .jpg (1536x1024)")
...
print(f"Generated {csv_file} with {len(unmatched_data)} flagged assets.")
